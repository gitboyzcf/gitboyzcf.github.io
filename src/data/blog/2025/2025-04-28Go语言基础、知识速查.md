...
tags:
  - 模板
description: Go（又称Golang） 由Google开发，于2009年首次公开发布。它旨在提供简洁、高效、可靠的软件开发解决方案。
modDatetime: 2026-01-12 15:27:45
---
...
### 注意点
- 一个目录只能属于一个包。多个文件可以是同一个包。
- 导出（可被其它包访问）的标识符必须以大写字母开头（如 `Greet`）。
- 包名通常与目录名相同且简短（如 `hello`、`http`），避免使用 `utils`/`common` 这种过泛名称。
- 避免包间循环导入（A→B且B→A 会报错）。
...
## 并发安全和锁
当进行并发协程同时操作一个资源时，导致两者出现竞争，最后输出结果不一，称为**并发不安全**

这是为什么呢？

> 根本原因是CPU的调度方法为抢占式执行，随机调度

怎样才能做到并发安全呢？可以使用**锁（Lock）** 的方式
...
```
可以看出两个协程函数`add、reduce` 对`num`同时去加减操作，正常情况下结果应该为 0，但是每次运行结果都不一样。
...
### 读写互斥锁
当我们读取一个资源时没有对资源进行写操作，不需要加锁；反之，有读取也有写入操作时可以使用**读写互斥锁**
...
<font color=red>concurrent map read and map write</font>
...
sync.Map内置了诸如`Store、Load、LoadOrStore、Delete、Range`等操作方法。
...
Go Module（go mod）是Go语言官方依赖管理工具，从`Go 1.11`版本开始引入，取代GOPATH模式，用于管理项目依赖、版本控制和模块发布。
‌
## 初始化模块
...
```
## 添加依赖
当你在代码中导入外部包并运行`go build`或`go run`时，Go会自动下载依赖并记录到`go.mod`文件中。
```bash
go get 包名
...
## 依赖管理常用命令
日常开发中常用的`go mod`命令包括：

- **go mod tidy**：清理未使用的依赖，补全缺失的依赖
- **go mod download**：下载所有`go.mod`中的依赖
